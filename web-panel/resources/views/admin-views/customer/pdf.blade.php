...
<head>
    ...
    <meta charset="UTF-8">
    <title>{{ \App\CPU\translate('Customer List') }}</title>
        ...
        <table style="width: 100%;">
            <tr>
                ...
                <td style="vertical-align: {{ $hasFilter ? 'top' : 'middle' }};">
                    <h4 style="font-size: 21px; font-weight: bold; margin:0;">{{ \App\CPU\translate('Customer List') }}</h4>
    ...
    <table style="width: 100%; font-size: 10px; border-collapse: collapse; color: #303030;">
        ...
        <tr style="background: #FAFAFA; border-top: 1px solid #EDF1F5; border-bottom: 1px solid #EDF1F5;">
            @foreach($headerRow as $header)
                <th style="padding: 6px; text-align: left; vertical-align: top; white-space: nowrap;">{{ \App\CPU\translate($header) }}</th>
            @endforeach
        </tr>
        </thead>
            ...
            <tr>
                @foreach($headerRow as $header)
                    <td style="padding: 10px 6px; text-align: left; vertical-align: top; white-space: nowrap; border-bottom: 0.5pt solid #EDF1F5;">
                        @if($header == 'image')
                            <img src="{{ (!empty($row['image']) && $row['image'] != 'def.png') ?  storage_path('app/public/customer/' . $row['image']) : public_path('assets/admin/img/160x160/img2.jpg') }}"
