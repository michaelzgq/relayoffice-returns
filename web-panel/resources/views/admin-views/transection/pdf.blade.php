...
<head>
    ...
    <meta charset="UTF-8">
    <title>{{ translate($pdfFor . ' List') }}</title>
...
<div style="max-width: 800px; margin: auto; position: relative; min-height: 11.69in;">
    ...
    <div style="margin-bottom: 30px; border-bottom: 1px dashed #E6E7EC; padding-bottom: 10px;">
        <table style="width: 100%;">
            <tr>
                @php
                    $filterKeys = ['search', 'sorting_type', 'start_date', 'end_date', 'account_id', 'transaction_type'];
                    $hasFilter = collect($filterKeys)->contains(fn($key) => request()->filled($key));
                @endphp
                <td style="vertical-align: {{ $hasFilter ? 'top' : 'middle' }};">
                    <h4 style="font-size: 21px; font-weight: bold; margin:0;">{{ translate($pdfFor . ' List') }}</h4>
                    @if(request()->filled('sorting_type'))
                        <p style="color: #646B73; margin:0;">
                            <strong>{{ translate('Sorting Type') }}
                                :</strong> {{ ucwords(request()->get('sorting_type')) }}
                        </p>
                    @endif
                    @if(request()->filled('search'))
                        <p style="color: #646B73; margin:0;">
                            <strong>{{ translate('Search') }}:</strong> {{ ucwords(request()->get('search')) }}
                        </p>
                    @endif
                    ...
                    @if(request()->filled('account_id'))
                        <p style="color: #646B73; margin:0;">
                            <strong>{{ translate('Account') }}:</strong> {{ ucwords($account) }}
                        </p>
                    @endif
                </td>
                <td style="text-align: right;">
                    ...
                    @endphp
                    <img src="{{(isset($shopLogo) && $shopLogo != 'def.png') ?  $logoPath : $logoFallback }}"
                         style="height: 45px;" alt="Shop Logo"/>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table style="width: 100%; font-size: 10px; border-collapse: collapse; color: #303030;">
        ...
        <tr style="background: #FAFAFA; border-top: 1px solid #EDF1F5; border-bottom: 1px solid #EDF1F5;">
            @foreach($headerRow as $header)
                <th style="padding: 6px; text-align: left; vertical-align: top; white-space: nowrap;">{{ translate($header) }}</th>
            @endforeach
        </tr>
        </thead>
...
<htmlpagefooter name="last-page-footer">
    <div style="border-top: 1px dashed #E6E7EC; padding-top: 6px; font-size: 9px; text-align: center;">
        <p style="margin: 6px 0;">{{ translate('Thanks for using our service.') }}</p>
