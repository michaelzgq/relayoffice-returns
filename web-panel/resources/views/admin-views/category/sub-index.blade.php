...
@section('content')
    <div class="content container-fluid">
            ...
            <div class="row align-items-center mb-3">
                <div class="col-sm">
                    <h1 class="page-header-title d-flex align-items-center g-2px text-capitalize mb-0">
                        <span>{{ \App\CPU\translate('sub_category_setup') }}</span>
                    </h1>
                </div>
            </div>
        </div>
        <div class="card mb-4">
            <div class="card-body">
                <div class="mb-4">
                    <h3 class="mb-0">{{ \App\CPU\translate('add_new_sub_category') }}</h3>
                    <p class="fs-12 mb-0">{{ \App\CPU\translate('Create a new product Subcategory to keep your items more organized and easy to manage.') }}</p>
                </div>
                <form action="{{route('admin.category.store')}}" method="post" id="store-or-update-data">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="">
                                            <label for="" class="title d-flex g-2px">
                                                {{\App\CPU\translate('sub_category')}} {{\App\CPU\translate('name')}}
                                                <span
                                                    class="text-danger">*</span>
                                            </label>
                                            <input type="text" name="name" class="form-control"
                                                   placeholder="{{\App\CPU\translate('type_sub_category_name')}}"
                                                   required>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="">
                                            <label class="title d-flex g-2px"
                                                ...
                                                <i class="fi fi-sr-info cursor-pointer ml-1 text-body"
                                                   data-toggle="tooltip"
                                                   title="{{ \App\CPU\translate('Select the main category to group this subcategory under a category for better product organization.') }}"></i>
                                            </label>
                                            <select id="exampleFormControlSelect1" name="parent_id"
                                                    class="form-control" required>
                                                <option value="" selected
                                                        disabled>{{\App\CPU\translate('select')}}</option>
        ...
        <div>
            <h4 class="align-items-center d-flex gap-2 mb-3">
                {{ \App\CPU\translate('sub_category_table') }}
        ...
        <div class="card">
                        ...
                        <form action="{{ url()->current() }}" method="GET">
                            ...
                            <div class="input-group input-group-merge input-group-flush">
                                ...
                                <input id="datatableSearch_" type="search" name="search" class="form-control"
                                       placeholder="{{ \App\CPU\translate('search_by_sub_category_name') }}"
                                    ...
                                    <div class="card card-sm">
                                        ...
                                        <div class="card-body">
                                            <div class="overflow-y-auto max-h-100vh-500px max-h-lg-100vh-400px">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center mb-3">
                                                    <span class="mr-2 fs-13 title text-capitalize">{{ \App\CPU\translate('Sub Category Name') }}</span>
                                                    <label class="toggle-switch toggle-switch-sm"
                                                ...
                                                <div class="d-flex justify-content-between align-items-center mb-3">
                                                    ...
                                                    <label class="toggle-switch toggle-switch-sm"
                                                           for="toggleColumn_parent_name">
                                                        <input type="checkbox"
                                                               class="toggle-switch-input update-column-visibility"
                                                               id="toggleColumn_parent_name" checked>
                                                        <span class="toggle-switch-label">
                                                        <span class="toggle-switch-indicator"></span>
                                                        </span>
                                                    </label>
            ...
            <div class="table-responsive">
                <table
                    ...
                    <tr>
                        ...
                        <th data-column="name">{{ \App\CPU\translate('Sub_Category_Name') }}</th>
                        <th data-column="parent_name">{{ \App\CPU\translate('Category_Name') }}</th>
                        ...
                        <tr>
                            ...
                            <td data-column="action">
                                <div class="d-flex justify-content-center align-items-center gap-3">
                                    ...
                                    <button type="button"
                                            class="btn btn-outline-primary icon-btn offcanvas-toggle edit-resource"
                                            data-id="{{ $resource['id'] }}"
                                    ...
                                    <button type="button"
                                            class="btn btn-outline-danger icon-btn {{ $resource['product_count'] > 0 ? 'delete-resource-after-shifting': 'delete-resource' }}"
                                            data-id="{{ $resource['id'] }}"
                                            data-target="{{ $resource['product_count'] > 0 ? '#deleteModalWithShift' : '#deleteModal' }}"
                                            data-toggle="modal"
                                            data-title="{{ \App\CPU\translate('Are you sure to delete this sub category') }}?"
                                            data-subtitle="{{ \App\CPU\translate('If once you delete this sub category, you will lost this sub category data permanently.') }}"
            ...
            <div class="card-footer d-flex align-items-center gap-x-24 flex-wrap justify-content-end">                
                    {!! $resources->links() !!}                    
            </div>
    ...
    <span class="data-to-js"
          ...
          data-view-route="{{ route('admin.category.sub-category.render-view-canvas') }}"
          data-edit-route="{{ route('admin.category.sub-category.render-edit-canvas') }}"
          data-delete-after-shifting-route="{{ route('admin.category.sub-category.delete-after-shifting-modal') }}"
          data-delete-route="{{ route('admin.category.delete', ':id') }}"
          data-fetch-subcategories-route="{{ route('admin.category.subcategories') }}"
    >
...
@push('script_2')
    ...
    <script>
        ...
        {
            let parentId = $(`#resource-select`).val();
            $.ajax({
                url: $('.data-to-js').data('fetch-subcategories-route'),
                type: "GET",
                data: {
                    id: id,
                    parent_id: parentId,
                ...
                success: function (response) {
                    let childSelect = $('#child-resource');
                    childSelect.empty();
                    childSelect.append(new Option('Select Sub category', 0));
                    if (response.length > 0) {

                        response.forEach(function (child) {
                            childSelect.append(new Option(child.name, child.id));
                        });
                    }
                },
                error: function () {
                    console.error('Error fetching children categories');
                }
            });
        }
