    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body pt-0">
                <form method="POST" action="{{ route('admin.category.delete', $resource['id']) }}">
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="text-center">
                        <img width="80" height="80" src="{{ asset('assets/admin/img/delete.png') }}" alt=""
                             class="mb-4">
                        <h3 class="mb-0">{{ \App\CPU\translate('Need to Shift Sub Category to Another Sub Category') }}</h3>
                        <p class="mt-3">
                            {{ trans(key: 'This sub category has :productCount '.  \Illuminate\Support\Str::plural('product', $resource['product_count']) . '. Please shift them to another sub category before deleting', replace: ['productCount' => $resource['product_count']]) }}
                        </p>
                        <div class="mt-4 d-flex justify-content-between gap-3 flex-column flex-md-row">
                            <div class="flex-grow-1 text-start w-50">
                                <label for="">{{  \App\CPU\translate('Choose Category')}}</label>
                                <select name="resource_id" id="resource-select"
                                        class="form-control js-select2-custom">
                                    @foreach($resources as $parentResource)
                                        <option
                                            value="{{ $parentResource['id'] }}" {{ $resource?->parent && $parentResource['id'] == $resource?->parent?->id ? 'selected' : '' }}>{{ $parentResource['name'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="flex-grow-1 text-start w-50">
                                <label for="">{{ \App\CPU\translate('Choose Sub Category') }}</label>
                                <select name="resource_child_id" id="child-resource"
                                        class="form-control js-select2-custom">
                                </select>
                            </div>
                        </div>
