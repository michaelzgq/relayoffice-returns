@php
    ...
    /** @var Collection|Category[] $resources */
...
<div class="offcanvas-filter__header d-flex justify-content-between align-items-start border-bottom px-2 py-2">
    <div class="pl-3 py-2">
        <h4 class="title mb-0">{{ translate('Category') . ' #'. $resource->id }}</h4>
    </div>
        ...
        <div class="bg-fafafa p-3 p-lg-4 rounded-10">
            <div class="d-flex gap-2 align-items-start mb-4">
                <img width="58" height="58"
                     src="{{ onErrorImage($resource->image,asset('storage/category/' . $resource->image),asset('assets/admin/img/160x160/img2.jpg'), 'category') }}"
                     class="aspect-1 object-cover rounded">
            ...
            @if($resource->description)
                <div class="p-3 bg-white rounded">
                    <h5 class="mb-1 font-weight-medium">{{ translate('Description') }}</h5>
                    <p class="mb-0 des_text">
                        {{ $resource->description }}
                        <button type="button"
                                class="btn text-primary p-0 m-0 see_more_btn">{{ translate('See More') }}</button>
                    </p>
                </div>
            @endif
        </div>
