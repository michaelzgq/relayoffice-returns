@php
    ...
    $perPage = $perPage ?? Helpers::pagination_limit();
@endphp
@if ($paginator->total() > 0)
    <div class="card-footer d-flex align-items-center flex-wrap gap-2 justify-content-between border-1 border-top">
                ...
                <div class="d-inline-block">
                    <select name="per_page" class="custom-select w-auto custom-select-small h-25px"
                        ...
                        @foreach (Helpers::paginateValueNumberOptions($perPage) as $paginateOptions)
                            <option
                                value="{{ $paginateOptions }}" @selected($perPage == $paginateOptions)>{{ $paginateOptions }}
