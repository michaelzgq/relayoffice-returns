...
@section('content')
    ...
    <div class="pb-2">
        <div class="progress cursor-pointer" role="progressbar" aria-label="Grofresh Software Installation"
             ...
             data-bs-placement="top" data-bs-custom-class="custom-progress-tooltip" data-bs-title="{{\App\CPU\translate('Fourth Step')}}!"
    ...
    <div class="card mt-4 position-relative">
        ...
        <div class="p-4 mb-md-3 mx-xl-4 px-md-5">
            <div class="d-flex align-items-center column-gap-3 flex-wrap">
                <h5 class="fw-bold fs text-uppercase">{{\App\CPU\translate('Step 4')}}. </h5>
                <h5 class="fw-normal">{{\App\CPU\translate('Import Database')}}</h5>
            </div>
            <p class="mb-5">
                {{\App\CPU\translate('Your Database has been connected ! Just click on the section to automatically import database')}}
            </p>
            ...
            @if(session()->has('error'))
                <div class="row">
                    <div class="col-md-12">
                        <div class="alert alert-danger">
                            {{\App\CPU\translate('Your database is not clean, do you want to clean database then import')}}?
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center">
                    <a href="{{ route('force-import-sql') }}" class="btn btn-danger px-sm-5" onclick="showLoder()">
                        {{\App\CPU\translate('Force Import Database')}}</a>
                </div>
            @else
                <div class="text-center">
                    <a href="{{ route('import_sql',['token'=>bcrypt('step_5')]) }}" class="btn btn-dark px-sm-5"
                       onclick="showLoder()">{{\App\CPU\translate('Click Here')}}</a>
                </div>
            @endif
        </div>
    </div>
