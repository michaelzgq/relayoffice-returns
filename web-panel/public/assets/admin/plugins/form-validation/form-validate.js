...
$(function(){
    ...
    $('.custom-validation').on('submit', function(e){
        var valid = true;
        $('.error-wrapper-iti input:visible').each(function(){
            $(this).closest('.error-wrapper-iti').find('.error-msg').remove(); // remove old error

            if(!$(this).val()){
                $(this).closest('.error-wrapper-iti').append('<div class="error-msg text-right d-block" style="color:red;font-size:12px;margin-top:3px;">This field is required</div>');
                valid = false;
            }
        });
    ...
    $('.error-wrapper-iti input:visible').on('input', function(){
        if($(this).val().trim() !== ''){
            $(this).closest('.error-wrapper-iti').find('.error-msg').remove();
        }
    });

    // Reset button clears errors
    $('.custom-validation button[type="reset"]').on('click', function(){
